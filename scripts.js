...
function encrypt(){
    ...
    for (let elem of data){
        if (elem in binaryAlphabet){
            format_data += binaryAlphabet[elem]
        } else {
            format_data += elem
        }
    }

    document.getElementById("binaryOutput").value = format_data
...
function decrypt(){
    ...
    for (let elem of data){
        ...
        if (temp.length == 8){
            format_data += binaryToLetter[temp];
            temp = "";
    ...
    document.getElementById("textInput").value = format_data
